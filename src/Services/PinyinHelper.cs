...
namespace RecentWorkspaceWidget.Services
{
    ...
    {
        private static Encoding gb2312Encoding;
        private static bool encodingInitialized = false;

        private static Encoding GetGbEncoding()
        {
            if (encodingInitialized) return gb2312Encoding;
            try
            {
                // In .NET Core / .NET 5+, code pages need provider registration if available
